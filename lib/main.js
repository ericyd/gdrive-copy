  ...
  while (properties.remaining.length > 0 && timer.canContinue()) {
    ...
    do {
      try {
        ...
        GDriveService.log(ss, [err.message, err.fileName, err.lineNumber]);
      }
...
function saveState(fileList, logMessage, ss) {
  ...
  GDriveService.log(ss, [logMessage]);
}
...
function exponentialBackoff(func, errorMsg) {
  for (var n = 0; n < 6; n++) {
    try {
      ...
      GDriveService.log(null, [e.message, e.fileName, e.lineNumber]);
      if (n == 5) {
        GDriveService.log(null, [
          errorMsg,
          '',
          '',
          '',
...
function processFileList(
  ...
  while (items.length > 0 && timer.canContinue()) {
     ...
     */
    newfile = FileService.copyFile(item, map);

    /*****************************
     * Log result
     */
    if (newfile.id) {
      GDriveService.log(ss, [
        'Copied',
        newfile.title,
