...
FileService.prototype.handleLeftovers = function(userProperties, ss) {
  if (
    this.properties.leftovers &&
    this.properties.leftovers.items &&
    this.properties.leftovers.items.length > 0
  ) {
...
FileService.prototype.handleRetries = function(userProperties, ss) {
  if (this.properties.retries && this.properties.retries.length > 0) {
    // Commented out on 2018-01-05 because I don't think this is necessary
    // this.properties.destFolder = this.properties.retries[0].parents[0].id;
    this.processFileList(this.properties.retries, userProperties, ss);
  }
};
