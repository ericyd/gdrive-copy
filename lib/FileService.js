 ...
 */
FileService.prototype.handleLeftovers = function(
  properties,
  userProperties,
  ss
) {
...
FileService.prototype.handleRetries = function(properties, userProperties, ss) {
  properties.retries;
  if (properties.retries && properties.retries.length > 0) {
    // Commented out on 2018-01-05 because I don't think this is necessary
    // properties.destFolder = properties.retries[0].parents[0].id;
    this.processFileList(properties.retries, properties, userProperties, ss);
  }
};
...
FileService.prototype.processFileList = function(
  ...
  while (items.length > 0 && this.timer.canContinue()) {
    ...
    if (
      ...
      Util.logCopyError(ss, item.error, item, properties.timeZone);
      continue;
    }
    ...
    try {
      ...
      Util.logCopySuccess(ss, newfile, properties.timeZone);
    } catch (e) {
      properties.retries.push({
        id: item.id,
        title: item.title,
