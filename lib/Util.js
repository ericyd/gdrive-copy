 ...
 * @param {Error} error
 * @param {File} failedItem
 * @param {string} timeZone
 */
Util.logCopyError = function(ss, error, failedItem, timeZone) {
  var parentId =
    failedItem.parents && failedItem.parents[0] ? failedItem.parents[0].id : null;
  Util.log(ss, [
    Util.composeErrorMsg(error)[0],
    failedItem.title,
    FileService.getFileLinkForSheet(failedItem.id, failedItem.title),
    failedItem.id,
